...
import { ArrowLeft, CalendarDays, Clock, ServerCog } from "lucide-react";
...
export default async function ArchitectureDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const arch = data.find((a) => a.slug === slug);
  if (!arch) return notFound();

  let markdownContent = "";
  try {
    markdownContent = getMarkdown(`architectures/${slug}.md`);
  } catch (error) {
    markdownContent = "Could not load architecture content.";
  }
    ...
    <article className="min-h-screen py-20 px-6 max-w-4xl mx-auto animate-in fade-in duration-1000 slide-in-from-bottom-4">
      ...
      <header className="mb-12">
        <div className="flex flex-wrap items-center gap-4 text-neutral-500 text-sm font-medium mb-6">
          ...
          <div className="w-1 h-1 rounded-full bg-neutral-300"></div>
          <div className="flex items-center gap-2 text-neutral-900 bg-neutral-100 px-2 py-0.5 rounded border border-neutral-200">
            <ServerCog className="w-3.5 h-3.5" />
            <span className="font-semibold text-xs tracking-wide uppercase">Architecture</span>
          </div>
        </div>
      ...
      <div className="relative w-full aspect-video bg-neutral-100 rounded-[2rem] overflow-hidden mb-16 shadow-xl border border-neutral-200">
        <Image
          src={arch.thumbnail === "PLACEHOLDER" ? `https://placehold.co/1200x675/f5f5f5/171717?text=${encodeURIComponent(arch.title)}` : arch.thumbnail}
          alt={arch.title}
          fill
          unoptimized
